...
static constexpr size_t kMaxCommandLength = 80;
static char g_commandBuffer[kMaxCommandLength + 1];
...
void printHelp();
void listKeys();
int16_t findKeyIndex(const char *command);
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (c == '\n') {
      ...
      if (g_commandLength < kMaxCommandLength) {
...
void processCommand(char *line) {
  ...
  while (token != nullptr && count < kMaxTokens) {
    tokens[count++] = token;
  ...
  if (strcmp(cmd, "help") == 0) {
    ...
    listKeys();
  } else if (strcmp(cmd, "press") == 0 || strcmp(cmd, "pulse") == 0) {
    if (count < 2) {
    ...
    if (count >= 3) {
      char *endPtr = nullptr;
      unsigned long parsed = strtoul(tokens[2], &endPtr, 10);
      if (endPtr == tokens[2] || *endPtr != '\0') {
        Serial.println(F("ERR: invalid duration"));
        return;
      }
      if (parsed > 0) {
        duration = parsed;
...
int16_t findKeyIndex(const char *command) {
  for (size_t i = 0; i < kKeyCount; ++i) {
    if (strcasecmp(command, kKeyMap[i].command) == 0) {
