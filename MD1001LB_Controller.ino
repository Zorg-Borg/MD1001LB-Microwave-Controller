...
// The keypad matrix ordering below matches measurements taken on late-run
// MD1001LB control boards. Earlier firmware builds assumed a different key
// ordering which caused many commands to actuate the wrong physical button.
// Each entry maps a friendly command token to the row/column index in the
// `kRowPins`/`kColumnPins` arrays that reproduces the label listed.
static const KeyDefinition kKeyMap[] = {
  {"clock",         "Clock",             0, 0},
  {"6",             "6",                 0, 1},
  {"cook_time",     "Cook Time",         0, 2},
  {"defrost",       "Defrost",           1, 0},
  {"5",             "5",                 1, 1},
  {"veggie",        "Veggie",            1, 2},
  {"power",         "Power",             2, 0},
